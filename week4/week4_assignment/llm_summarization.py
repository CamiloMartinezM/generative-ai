...
if __name__ == "__main__":
    ...
    #MODEL_NAME = "course-genai-w24/week4-gpt2-sft-tldr"
    #MODEL_NAME = "course-genai-w24/week4-gpt2-lora-sft-tldr"
    ...
    all_summaries = write_summaries_to_file(reddit_posts, model, tokenizer, n_generations, device,
                                      f'output_reddit_summaries_{MODEL_NAME.split("/")[-1]}.txt')

    # Read reference summaries from file
    with open('reference_reddit_summaries.txt', 'r') as f:
        reference_summaries = f.read().splitlines()
    # Compute and output ROUGE scores
    compute_rouge_scores_and_write_to_file(all_summaries, reference_summaries, n_generations,
                                                          f'output_rouge_score_{MODEL_NAME.split("/")[-1]}.txt')
